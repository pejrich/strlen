...
impl SLRange {
    ...
    // }
    pub fn from_sta_len(start: isize, length: isize) -> Self {
        SLRange {
            start,
            length,
            stop: start + length - 1,
...
pub fn range_from_point(len: SLLength, point: Option<isize>) -> SLStringRange {
    SLStringRange::from_point(point.unwrap_or(0), len)
}
#[rustler::nif]
pub fn range_from_range(len: SLLength, range: SLStringRange) -> SLStringRange {
    SLStringRange::from_range(range, len)
}
...
fn replace(range: SLStringRange, string: &str) -> SLStringRange {
    SLStringRange::replace(range, SLLength::new(string))
}

#[rustler::nif]
fn shift_after(range: SLStringRange, prev: SLStringRange) -> SLStringRange {
    SLStringRange::shift_after(range, prev)
}

rustler::init!("Elixir.StrLen.Native");
